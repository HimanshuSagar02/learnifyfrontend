...
function EnrolledCourse() {
  ...
  }, [courses, userData?._id]);

  return (
    <div className="min-h-screen px-4 sm:px-6 py-10 bg-gray-50">

      <FaArrowLeftLong className="fixed left-4 sm:left-6 top-24 sm:top-6 text-xl cursor-pointer z-10"
        onClick={() => navigate("/")}
      />
      ...
      {courses.length === 0 ? (
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {courses.map((course) => (
            <div key={course._id}
              className="bg-white w-full max-w-[320px] shadow rounded-xl overflow-hidden">

              <img src={course.thumbnail} alt={course.title || "Course"} className="w-full h-40 object-cover" />
