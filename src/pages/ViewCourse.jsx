...
const normalizedCoursePrice = Number(selectedCourseData?.price);
const hasPaidPrice = Number.isFinite(normalizedCoursePrice) && normalizedCoursePrice > 0;
const isFreeCourse = Boolean(selectedCourseData) && !hasPaidPrice;
...
useEffect(() => {
  if (creatorData?._id && courseData.length > 0) {
    const creatorCourses = courseData.filter(
      (course) => {
        const creatorId = typeof course.creator === "string" ? course.creator : course.creator?._id;
...
const handleEnroll = async (courseId, userId) => {
  try {
    if (!selectedCourseData?._id) {
      toast.error("Course details are loading. Please try again.");
      return;
    }
    ...
    if (isFreeCourse) {
      ...
      if (enrollRes.data?.alreadyEnrolled) {
        toast.info("You are already enrolled in this course");
      } else {
        toast.success(enrollRes.data?.message || "Successfully enrolled in the course");
      }
      setIsEnrolled(true);
      return;
    }
        ...
        <div className="flex flex-col md:flex-row gap-6 ">
          ...
          <div className="flex-1 space-y-2 mt-[20px]">
            ...
            <div className="flex items-start flex-col justify-between">
              ...
              <div>
                <span className="text-lg font-semibold text-black">
                  {hasPaidPrice ? `INR ${normalizedCoursePrice}` : "Free"}
                </span>{" "}
                {hasPaidPrice && <span className="line-through text-sm text-gray-400">INR 599</span>}
              </div>
            </div>
            ...
            {userData?.role === "student" && (
              !isEnrolled ? (
                <button className="bg-[black] text-[#3B82F6] px-6 py-2 rounded hover:bg-gray-800 border border-[#3B82F6] mt-3" onClick={()=>handleEnroll(courseId , userData._id)}>
                  Enroll Now
                </button>
              ) : (
    ...
    <div className="mb-4">
      <div className="flex gap-1 mb-2">
        {[1, 2, 3, 4, 5].map((star) => (
            ...
            <FaStar  key={star}
            onClick={() => setRating(star)} className={star <= rating ? "fill-blue-500" : "fill-gray-300"} />
      ...
      <button
        ...
        className="bg-black text-[#3B82F6] mt-3 px-4 py-2 rounded hover:bg-gray-800 border border-[#3B82F6]" onClick={handleReview}
      >
        Submit Review
      </button>
    </div>
